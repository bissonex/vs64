...
export default function KeyBoard(vscode) {
    var _vscode = this.vscode;
            ...
            function _mousedown(ev) {
                ...
                if (key.length > 1) {
                    ...
                    if (controlled && key >= '@' && key <= '_') {
                        ...
                        vscode.postMessage({ type: 'keyPressed', value: key.charCodeAt(0) - 0x40 });
                    } else if (!shifted && !capslocked &&
                               key >= 'A' && key <= 'Z') {
                        //io.keyDown(key.charCodeAt(0) + 0x20);
                        vscode.postMessage({ type: 'keyPressed', value: key.charCodeAt(0) + 0x20 });
                    } else {
                        //io.keyDown(key.charCodeAt(0));
                        vscode.postMessage({ type: 'keyPressed', value: key.charCodeAt(0) });
                    }
                }
            }
